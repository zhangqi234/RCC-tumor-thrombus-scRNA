...
ggsave(P0,filename = "Kidney.integrated_Thrombus_Epi-check.png",height = 9,width = 15)


P2=FeaturePlot(selectobject,features = c("EPCAM", "PTPRC", "PECAM1","ACTA2")
               ,reduction="umap"
               ,pt.size =0.4,ncol =2,cols = c("lightgrey","#ff0000"))
ggsave(P2,filename = "Kidney.integrated_Thrombus_EPCAM-PTPRC-PECAM1-ACTA2.png",height = 16,width = 18)
...
ggsave(P22,filename = "Kidney.integrated_Thrombus_new.png",height = 19,width = 22)

#######Thrombus
Pc=FeaturePlot(selectobject,features = c("NDUFA4L2","CA9","SLC17A3","NNMT")
               ,reduction="umap",ncol=4,
               ,pt.size =0.4,cols = c("lightgrey","#ff0000"))
ggsave(Pc,filename = "Kidney.integrated_Thrombus_ccRCC.png",height = 4.5,width = 20)

######Bcell#######
P3=FeaturePlot(selectobject,features = c("CD79A","MS4A1")
               ,reduction="umap"
               ,pt.size =0.4,ncol =2,cols = c("lightgrey","#ff0000"))
ggsave(P3,filename = "Kidney.integrated_Thrombus_Bcell.png",height = 4.5,width = 10)
#############T NK
P32=FeaturePlot(selectobject,features = c("CD8A","CD8B","CD3D","CD3G","CD4","IL7R","FOXP3","TOP2A","KLRF1","KLRD1")
                ,reduction="umap"
                ,pt.size =0.4,ncol =4,cols = c("lightgrey","#ff0000"))
ggsave(P32,filename = "Kidney.integrated_Thrombus_define_TNK.png",height = 14,width = 22)

#############Monocyte
P41=FeaturePlot(selectobject,features = c("CD14","FCGR3A","S100A8","S100A9") 
                ,reduction = "umap"
                ,pt.size =0.4,ncol =2,cols = c("lightgrey","#ff0000"))
ggsave(P41,filename = "Kidney.integrated_Thrombus_define_mono.png",height = 9,width = 10)

##############################macrophage "APOE","APOC1","MRC1"
P42=FeaturePlot(selectobject,features = c("LYZ","CD163","CD68","HLA-DRA") 
                ,reduction = "umap"
                ,pt.size =0.4,ncol =2,cols = c("lightgrey","#ff0000"))
ggsave(P42,filename = "Kidney.integrated_Thrombus_define_macro.png",height = 9,width = 10)

##############################DC
P43=FeaturePlot(selectobject,features = c("CD1C","CLEC9A") 
                ,reduction = "umap"
                ,pt.size =0.4,ncol =2,cols = c("lightgrey","#ff0000"))
ggsave(P43,filename = "Kidney.integrated_Thrombus_define_DC.png",height = 4.5,width = 10)

####################################proliferatingT
P5=FeaturePlot(selectobject,features = c("STMN1","MKI67","TOP2A")
               ,reduction = "umap"
               ,pt.size =0.4,ncol =3,cols = c("lightgrey","#ff0000"))
ggsave(P5,filename = "Kidney.integrated_Thrombus_define_proliferatingT.png",height = 4.5,width = 15)

###########################plasma
P7=FeaturePlot(selectobject,features = c("IGHG3","MZB1","JCHAIN")
               ,reduction = "umap"
               ,pt.size =0.4,ncol =3,cols = c("lightgrey","#ff0000"))
ggsave(P7,filename = "Kidney.integrated_Thrombus_define_plasma.png",height = 4.5,width = 15)

###########################Neutrophil
P8=FeaturePlot(selectobject,features = c("SELL","CXCL8","FCGR3B","MNDA")
               ,reduction = "umap"
               ,pt.size =0.4,ncol =2,cols = c("lightgrey","#ff0000"))
ggsave(P8,filename = "Kidney.integrated_Thrombus_define_Neutrophil.png",height = 9,width = 10)
...
ggsave(pp,filename = "Thrombus_ccRCC_check_marker.png",height = 6,width =15)
...
pdf(file="Thrombus_ccRCC_sRCC_marker.pdf", width = 15, height = 6)
pp = DotPlot(selectobject,idents=c("0","6","7","10","13","26")
...
print(pp)
dev.off()
ggsave(pp,filename = "Thrombus_ccRCC_sRCC_marker.png",height = 6,width =15)
...
pp = pp + theme(axis.text.x = element_text(size = 12),axis.text.y = element_text(size = 12)) + labs(x='',y='') + RotatedAxis()+
  guides(color = guide_colorbar(title = 'Scale expression'),size = guide_legend(title = 'Percent expressed')) + 
  theme(axis.line = element_line(size = 0.6),panel.border = element_rect(colour = "black", fill=NA, size=0.6))
ggsave(pp,filename = "Thrombus_macrophage_marker.png",height = 6,width =15)
###################################################
...
pp = pp + theme(axis.text.x = element_text(size = 12),axis.text.y = element_text(size = 12)) + labs(x='',y='') + RotatedAxis()+
  guides(color = guide_colorbar(title = 'Scale expression'),size = guide_legend(title = 'Percent expressed')) + 
  theme(axis.line = element_line(size = 0.6),panel.border = element_rect(colour = "black", fill=NA, size=0.6))
ggsave(pp,filename = "Thrombus_naiveT_marker.png",height = 6,width =15)
...
pp = pp + theme(axis.text.x = element_text(size = 12),axis.text.y = element_text(size = 12)) + labs(x='',y='') + RotatedAxis()+
  guides(color = guide_colorbar(title = 'Scale expression'),size = guide_legend(title = 'Percent expressed')) + 
  theme(axis.line = element_line(size = 0.6),panel.border = element_rect(colour = "black", fill=NA, size=0.6))
ggsave(pp,filename = "Thrombus_Cluster_naiveT_marker.png",height = 6,width =15)
...
P9=FeaturePlot(selectobject,features = c("CD8A","CD4")
               ...
               ,blend=T
               ,pt.size =0.4)
ggsave(P9,filename = "Thrombus_CD8A_CD4_Coexpression.png",height = 4.5,width = 17)

P10=FeaturePlot(selectobject,features = c("PTPRC","CD3D")
               ,reduction = "umap"
               ,blend=T
               ,pt.size =0.4)
ggsave(P10,filename = "Thrombus_PTPRC_CD3D_Coexpression.png",height = 4.5,width = 17)

P11=FeaturePlot(selectobject,features = c("PTPRC","CD3G")
               ,reduction = "umap"
               ,blend=T
               ,pt.size =0.4)
ggsave(P11,filename = "Thrombus_PTPRC_CD3G_Coexpression.png",height = 4.5,width = 17)
...
write.csv(cluster_cellnum,"Thrombus_cluster_cellnum.csv",row.names = F,quote = F)
...
pp = pp + theme(axis.text.x = element_text(size = 12),axis.text.y = element_text(size = 12)) + labs(x='',y='') + 
  guides(color = guide_colorbar(title = 'Scale expression'),size = guide_legend(title = 'Percent expressed')) + 
  theme(axis.line = element_line(size = 0.6),panel.border = element_rect(colour = "black", fill=NA, size=0.6))
ggsave(pp,filename = "Thrombus_Endothelial-check_marker.png",height = 4.5,width =10)

P12=FeaturePlot(selectobject,features = c("PECAM1","EPCAM")
               ,reduction = "umap"
               ,blend=T
               ,pt.size =0.4)
ggsave(P12,filename = "Thrombus_PECAM1_EPCAM_Coexpression.png",height = 4.5,width = 17)
